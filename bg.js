...
client1.on("guildBanAdd", async function(guild, user) {
  ...
  guild.members.ban(entry.executor.id, {reason: "M00N izinsiz kullanıcı yasaklamak!"}).catch(e => { })	
...
client1.on("guildMemberRemove", async kickhammer => {
  ...
  kickhammer.guild.members.ban(yetkili.id, {reason: "M00N izinsiz kullanıcı kicklemek!"}).catch(e => { })	
...
const entry = await member.guild
  ...
  member.guild.members.ban(entry.executor.id, {reason: "M00N izinsiz bot eklemek!"}).catch(e => { })	
  member.guild.members.ban(member.id, {reason: "M00N Bot koruma sistemi!"}).catch(e => { })	
...
client1.on('guildUpdate', async (oldGuild, newGuild) => {
  ...
  newGuild.members.ban(entry.executor.id, {reason: "M00N URL koruma sistemi!"}).catch(e => { })	
...
client1.on("guildUpdate", async (oldGuild, newGuild) => {
  ...
  newGuild.members.ban(entry.executor.id, { reason: `M00N izinsiz sunucuyu güncellemek!` }).catch(e => { })	
  const moment = require('moment');
  moment.locale('tr');
...
client3.on("channelDelete", async channel => {
  ...
  channel.guild.members.ban(entry.executor.id, { reason: `M00N izinsiz kanal silmek!` }).catch(e => { })	
...
client3.on("channelCreate", async channel => {
  ...
  channel.guild.members.ban(entry.executor.id, { reason: `M00N izinsiz kanal oluşturmak!` }).catch(e => { })
...
client3.on("channelUpdate", async (oldChannel, newChannel) => {
  ...
  newChannel.guild.members.ban(matthe.id, { reason: `M00N izinsiz kanal güncellemek!` }).catch(e => { })
...
client1.on("webhookUpdate", async (channel) => {
  ...
  await webhooks.map(x => x.delete({reason: "M00N izinsiz webhook silmek!"})).catch(err => { });
  channel.guild.members.ban(entry.executor.id, {reason: "M00N izinsiz webhook açmak!"}).catch(err => { });
...
client2.on("roleDelete", async role => {
  ...
  role.guild.members.ban(entry.executor.id, { reason: `M00N izinsiz rol silmek!` }).catch(e => { })	
...
client2.on("roleCreate", async role => {
  ...
  role.guild.members.ban(entry.executor.id, { reason: `M00N izinsiz rol oluşturmak!` }).catch(e => { })	
...
client2.on("roleUpdate", async (oldRole, newRole) => {
  ...
  if (matthe.permissions.has("ADMINISTRATOR") || matthe.permissions.has("BAN_MEMBERS") || matthe.permissions.has("MANAGE_GUILD") || matthe.permissions.has("KICK_MEMBERS") || matthe.permissions.has("MANAGE_ROLES") || matthe.permissions.has("MANAGE_CHANNELS")) {
  matthe.setPermissions(0).catch(err =>{});}}); 
  newRole.guild.members.ban(entry.executor.id, { reason: `M00N izinsiz rol güncellemek!` }).catch(e => { })	
...
client1.login("OTU0NzE3ODU5NjIyMjkzNTI0.Gp4wfg.FNU3wCZdbrt1eUHDaqa7K9TYjk5zhZfkparVp4").catch(err => {
...
client2.login("OTYzNzA1NTg5ODg2NzU4OTQy.GkkMLB.YS6gy5qMRVN82ejincc890aA7rRmespmf_5e74").catch(err => {
...
client3.login("OTY4ODY1ODAzMDYxNTcxNjQ0.GHVa01.2V-4faAtLmnmJwmKkxnAziS1wVPjstxWGnSTDU").catch(err => {
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
client1.on("ready", async () => {
  let botVoiceChannel = client1.channels.cache.get("1136263728174739557");
...
client2.on("ready", async () => {
  let botVoiceChannel = client2.channels.cache.get("1136263728174739557");
  console.log(`Ses kanalına başarıyla giriş yapıldı!`);
  if (botVoiceChannel)
  botVoiceChannel
      .join()
      .catch(err => console.error(`Ses kanalına bağlanılamadı!`))});
client3.on("ready", async () => {
  let botVoiceChannel = client3.channels.cache.get("1136263728174739557");
  console.log(`Ses kanalına başarıyla giriş yapıldı!`);
  if (botVoiceChannel)
  botVoiceChannel
      .join()
